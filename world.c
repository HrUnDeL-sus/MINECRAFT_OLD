...
for(int x=0;x<16;x+=1){
    for(int z=0;z<16;z+=1){
        block local_block=get_chunk->chunk_blocks[x][pos.y>255?255:(int)pos.y][z];
        if(local_block.pos_x==(int)pos.x&&local_block.pos_z==(int)pos.z)
...
//printf("\nDIRECTION:%f %f",direction.x,direction.y);
        ...
        for(int x=0; x<count_chunks; x+=1)
        {
            for(int z=0; z<count_chunks; z+=1)
            {
                ...
                if(main_config.use_threads==0)
                {
                    if(chunk_in_world[x][z].was_modified==1&&can_modify==1)
                        pre_rendering_chunk(&chunk_in_world[x][z]);
                    else if(can_modify==0)
                         pre_rendering_chunk(&chunk_in_world[x][z]);
                    z1-=1;
                    continue;
                }
